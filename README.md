...
知识库支持绑定独立的 embedding 配置版本，当前 HNSW 索引支持 1～4000 维，要求 pgvector 0.8 或更高版本。同一 PostgreSQL 实例可存储不同维度；不同模型分别生成查询向量，再融合检索结果。

升级前先备份数据库，以 `DB_SYNC=0` 运行手写迁移，再启动新后端：
...
- 在「设置 → 模型管理」配置 embedding 模型；输出维度留空时，在绑定知识库时自动探测。只有供应商支持指定维度时才打开对应开关。
- 在知识库「设置 → 向量配置」中选择模型并重建。所有最新分块完成向量化、索引就绪后，由创建者手动切换。
- 未绑定配置的旧知识库保留关键词检索，不猜测旧向量来源。迁移不删除原 `chunks.embedding` 列，也不截断、补零或自动冒认旧向量。
- 更改用户默认模型不会影响已绑定知识库。已被配置版本引用的模型允许轮换凭据；更换模型或维度请新增模型并重建。
- 切换后保留上一配置并同步后续内容修改，用于覆盖校验后的回滚；维护上一配置会产生额外 embedding 调用。
- 数据库触发器在正文提交时创建向量任务，后台可靠投递、分批处理；失败重试复用已完成批次。

测试入口：`npm --workspace backend test`。真实 PostgreSQL 测试仅在显式设置 `MULTIDIM_TEST_DATABASE_URL` 时运行，该地址必须指向独立测试库；测试不读取应用 `.env`。
